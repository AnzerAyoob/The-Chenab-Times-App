...
class CorsProxyService {
  ...
  static Future<String?> fetchWithProxy(String targetUrl) async {
    ...
    for (int i = 0; i < _publicProxies.length; i++) {
      ...
      final proxyUrl = '${_publicProxies[proxyIndex]}${Uri.encodeComponent(targetUrl)}';
  ...
  static Future<bool> isLocalProxyAvailable() async {
    try {
      final response = await http.get(
        Uri.parse('http://localhost:8089/raw?url=test'),
      ).timeout(const Duration(seconds: 2));
      return response.statusCode != 404; // 404 means proxy is running but URL is invalid
    } catch (e) {
      return false;
    }
  }
